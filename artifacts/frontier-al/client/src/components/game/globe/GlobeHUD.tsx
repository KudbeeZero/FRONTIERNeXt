...
export function GlobeHUD({ activeBattleCount, replayTime }: GlobeHUDProps) {
  ...
  // Derive display values from chain health
  const chainOk = chainHealth?.ok ?? null;
  const chainColor = chainOk === null
    ? "rgba(0,229,255,0.35)"   // unknown — muted cyan (no data yet)
    : chainOk
      ? "#00ff6a"               // healthy — green
      : "#ff1744";              // degraded — red
  const chainLabel = chainOk === null ? "CHAIN: —" : chainOk ? "CHAIN: NOMINAL" : "CHAIN: DEGRADED";
  const chainRound = chainHealth?.algorand?.lastRound
    ? `BLK ${chainHealth.algorand.lastRound.toLocaleString()}`
    : null;
  ...
  return (
    <>
      ...
      {/* Bottom-right telemetry + chain health indicator */}
      <div
      ...
      >
        ...
        {/* Chain health row — appears below the static telemetry */}
        <div
          style={{
            marginTop: 4,
            display: "flex",
        ...
        >
          <span
            style={{
              ...
              boxShadow: chainOk ? `0 0 6px ${chainColor}` : undefined,
              animation: chainOk === null ? undefined : "pulse 2s infinite",
            }}
          />
          <span style={{ color: chainColor, letterSpacing: "0.2em" }}>{chainLabel}</span>
          {chainRound && (
            <span style={{ color: "rgba(0,229,255,0.35)", marginLeft: 4 }}>{chainRound}</span>
          )}
        </div>
      </div>
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-2 text-center flex-shrink-0">
          {[
            ...
            { label: "ASCEND/d",  value: parcel.ascendPerDay?.toFixed(1) ?? "—" },
          ].map(({ label, value }) => (
