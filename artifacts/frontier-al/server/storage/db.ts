  ...
  async executeSpecialAttack(action: SpecialAttackAction): Promise<{ damage: number; effect: string }> {
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (action.attackType === "orbital_strike") {
        ...
        (targetUpdates as any).empDebuffUntil = Date.now() + 10 * 60 * 1000;
        effectDescription = "EMP disabled turrets and shields — defense –2 for 10 min";
        ...
        (targetUpdates as any).sabotageDebuffUntil = Date.now() + 30 * 60 * 1000;
        effectDescription = "Sabotage halved target mining yield for 30 min";
      }
      ...
      await this.db.transaction(async (tx) => {
        ...
        if (attackerWins) {
          ...
          if (battleRow.commanderId) {
            updatedCommandersForWin = (attackerRow.commanders as any[] ?? []).map((c: any) =>
              c.id === battleRow.commanderId
                ? { ...c, totalKills: (c.totalKills ?? 0) + 1 }
                : c
            );
          }
          ...
          await Promise.all([
            ...
            tx.update(playersTable)
              .set({
                ...
                commanders:          updatedCommandersForWin,
              })
