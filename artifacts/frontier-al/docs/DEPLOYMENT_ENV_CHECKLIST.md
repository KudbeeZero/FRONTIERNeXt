...
| `ASCEND_ASA_ID` **[REQ][SEC]** | **Pins the ASCEND token ASA ID.** Positive integer (the Algorand asset ID). When set, the server uses this ID directly at startup instead of doing a name-based on-chain lookup. Validated at boot by `assertChainConfig()` — invalid values fail fast. **Strongly recommended in production** to prevent accidental ASA drift on restart. Example: `755818217` (TestNet). |
