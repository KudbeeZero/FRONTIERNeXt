...
| ASCEND_ASA_ID | **Pins the ASCEND token ASA ID.** Positive integer (the Algorand asset ID). When set, the server uses this ID directly at startup instead of doing a name-based on-chain lookup. Validated at boot by `assertChainConfig()` — invalid values (non-integer, zero, negative) fail fast. **Strongly recommended in production** to prevent accidental ASA drift on restart. | `755818217` (TestNet) |
...
| HALT_DB | **Database kill switch.** `true` halts ALL database reads/writes AND background tasks. `/api` routes return 503; `/health` stays 200. Only the literal string `"true"` activates it (unset/`false`/`1`/any-other-value = normal operation). Use to put the DB in dormant zero-data-transfer state while keeping the app live. Reversible: set to any other value or remove. | unset → normal · `true` → dormant |
...
| VITE_DEV_AUTOLOGIN | **Local iteration tool ONLY — zero-click.** `true` makes the landing page run the dev quick-auth on load and enter the game with **no button click**. Triple-gated: requires `VITE_DEV_MODE=true` too AND the server's `DEV_LOGIN_ENABLED`. **Do NOT set this in any deployed environment (`fly.toml` or otherwise)** — it hijacks every visitor, including ones with a real wallet, into the shared non-spendable dev/test identity (fixed 2026-07-06 after it broke real-wallet login/sign-out/purchases in production). Use it only as a local env var for fast local dev. | unset everywhere except a local shell |
