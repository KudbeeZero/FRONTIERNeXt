 ...
 * Costs are derived from each spec's own `costAscend` (the per-shot fire cost),
...
/** In testing mode all ASCEND costs are scaled down for frictionless partner testing. */
export const WEAPON_TEST_DISCOUNT = 0.25;

function scale(ascend: number): number {
  return ECONOMY_MODE === "production"
    ? Math.round(ascend)
    : Math.max(1, Math.round(ascend * WEAPON_TEST_DISCOUNT));
}

/** ASCEND to fire one shot of this weapon. */
export function fireCostAscend(spec: WeaponSpec): number {
  return scale(spec.costAscend);
}

/** One-time ASCEND to unlock/acquire this weapon into the player's armory. */
export function unlockCostAscend(spec: WeaponSpec): number {
  return scale(spec.costAscend * WEAPON_UNLOCK_COST_MULT);
}

/** ASCEND to upgrade an owned weapon to the given (1-based) tier. */
export function upgradeCostAscend(spec: WeaponSpec, toTier: number): number {
  return scale(spec.costAscend * WEAPON_UPGRADE_COST_MULT * Math.max(1, toTier));
}

/** ASCEND to deploy a defensive battery onto a parcel. */
export function deployCostAscend(spec: WeaponSpec): number {
  return scale(spec.costAscend * DEFENSE_DEPLOY_COST_MULT);
}
