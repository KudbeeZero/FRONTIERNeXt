...
- **Active lane (Phase 4A, server-side plot-attack idempotency):** `fix/frontier-plot-attack-idempotency` → PR **OPEN** to `main` (title `fix: make plot attacks server-idempotent`). Reuses the existing DB-backed `action_nonces` guard; adds an additive `payload_fingerprint` column (migration `0015`) so same-key/different-payload replays are rejected 409. Scope limited to `POST /api/actions/attack`; `/archetype` + `/build` deferred to Phase 4B. Battle math/`resolveBattle()`/AI/wallet/chain untouched. Awaiting owner review/merge. See `docs/memory/FRONTIER_SUBPLOT_COMBAT_ARCHITECTURE.md` §Phase 4A.

## NEXT
- **Next lane:** (1) owner activates cost-control via `flyctl secrets set` (see blocker above) and observes 15 min; (2) **Phase 4A PR OPEN** (`fix/frontier-plot-attack-idempotency`) — review/merge; after merge, **Phase 4B** (`/archetype` + `/build` idempotency) is next, then Phase 5. Later phases stay one-PR-at-a-time per HARD RULES.
