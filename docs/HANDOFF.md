...
- **Active lane (Phase 3, immutable CombatProfile / battle-snapshot contract):** `feat/frontier-combat-profile` — pure, deterministic, immutable combat-profile + battle-snapshot contract in `shared/combatProfile.ts` (+ `shared/combatProfile.spec.ts`, 60 tests green). Zero gameplay effect; no attack route / DB / `resolveBattle()` / AI / UI wiring. PR open to `main`, awaiting owner review/merge. See `docs/memory/FRONTIER_SUBPLOT_COMBAT_ARCHITECTURE.md` §Phase 3.

## NEXT
- **Next lane:** (1) owner activates cost-control via `flyctl secrets set` (see blocker above) and observes 15 min; (2) **Phase 3 PR open** (`feat/frontier-combat-profile`, pure contract, awaiting merge) — after merge, Phase 4 (server-side idempotency for `/attack`) is next, pending owner architecture approval. Later phases stay one-PR-at-a-time per HARD RULES.
