...
- **Unit:** Strip credentials from mission-control `remoteUrl` (secret-leak fix) — **DONE & MERGED** (PR #278 `b09695d`, 2026-07-16).
  ...
  - **Process note:** first push accidentally had a `[skip ci]` commit as the branch tip → no CI ran. Corrected by reordering so the fix commit is the tip (CI-eligible), force-push-with-lease on the feature branch only. **Lesson: never leave a `[skip ci]` commit as a PR branch tip.**
