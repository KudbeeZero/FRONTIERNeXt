...
**Next unit:** M1-6 (DB indexes + rate limiter extension) — migration 0014 adds functional index on `players (lower(address))`, btree indexes on `players.is_ai`/`player_faction_id`, secondary indexes on `trade_orders.status`/`offerer_id`/`created_at`. Add `strictLimiter` (60/min, memory store) bound to `/api/trade/*`, `/api/markets/*`, `/api/weapons/*`, `/api/sub-parcels/*`, `/api/factions/*`. Add pagination `LIMIT 1000` to `queryPurchaseIntents()` in `chainEventStore.ts`. Add middleware-binding coverage test. Branch: `chore/db-indexes-ratelimit`. Open risks: none. Off-limits: standard hard rules.
